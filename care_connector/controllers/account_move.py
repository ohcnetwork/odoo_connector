...
from ..pydantic_models.account_move import AccountMoveApiRequest,AccountMoveReturnApiRequest,AccountMoveCancelApiRequest
...
class AccountMove(http.Controller):
    ...
    @http.route('/api/account/move/cancel', type='http', auth='public', methods=['POST'], csrf=False)
    def account_move_cancel(self, **kwargs):
        try:
            auth_header = request.httprequest.headers.get("Authorization")
            user_env = UserAuthentication.get_authenticated_user(auth_header)
            data = json.loads(request.httprequest.data)
            request_data = AccountMoveCancelApiRequest(**data)
