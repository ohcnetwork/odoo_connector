...
class CashSession(models.Model):
    ...
    external_user_name = fields.Char(
        string='User Name',
        required=True,
        help='Display name from Care system'
    )
    ...
    ], string='Variance Status', default='none')
    ...
    def _compute_display_name(self):
        for session in self:
            counter_name = session.counter_id.bill_counter if session.counter_id else 'Unknown'
            date_str = session.opened_at.strftime('%Y-%m-%d %H:%M') if session.opened_at else ''
            session.display_name = f"{session.external_user_name} @ {counter_name} ({date_str})"
    ...
    @api.depends('outgoing_transfer_ids', 'outgoing_transfer_ids.amount', 'outgoing_transfer_ids.status')
    ...
    def _compute_available_balance(self):
        ...
        for session in self:
            session.available_balance = session.expected_amount - session.pending_outgoing_amount
    ...
    def action_resolve_variance(self, status, note=None):
        ...
        if status not in ('recoverable', 'written_off'):
